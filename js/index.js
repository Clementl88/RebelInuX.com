...
/* ===== PROFESSIONAL INVESTOR ENHANCEMENTS ===== */

// Enhanced counter animations with currency formatting
function initProfessionalCounters() {
...
// Real-time market sentiment simulator (professional touch)
function initMarketSentiment() {
  const sentimentIndicators = document.querySelectorAll('.sentiment-indicator');
  
  if (sentimentIndicators.length === 0) return;
  
  // Simulate changing sentiment colors
  setInterval(() => {
    sentimentIndicators.forEach(indicator => {
      const sentiments = ['bullish', 'neutral', 'bearish'];
      const currentSentiment = indicator.classList[1];
      const newSentiment = sentiments[Math.floor(Math.random() * sentiments.length)];
      
      indicator.classList.remove('bullish', 'neutral', 'bearish');
      indicator.classList.add(newSentiment);
...
// Professional data refresh simulation
function initDataRefresh() {
  const refreshElements = document.querySelectorAll('[data-refresh]');
  
  refreshElements.forEach(element => {
    const interval = parseInt(element.getAttribute('data-refresh') || '10000');
    
    setInterval(() => {
      // Simulate data update with slight random variation
      if (element.classList.contains('stat-value')) {
        const baseValue = parseFloat(element.getAttribute('data-base')) || 0;
        const variation = baseValue * 0.02; // ±2%
        const newValue = baseValue + (Math.random() * variation * 2 - variation);
        ...
        if (baseValue >= 1000000) {
          element.textContent = prefix + (newValue / 1000000).toFixed(2) + 'M' + suffix;
        } else if (baseValue >= 1000) {
          element.textContent = prefix + (newValue / 1000).toFixed(1) + 'K' + suffix;
        } else {
...
// Professional network status indicator
function initNetworkStatus() {
  const statusElement = document.createElement('div');
  statusElement.className = 'network-status';
  statusElement.innerHTML = `
    <div class="status-indicator online"></div>
    <span class="status-text">Live Network</span>
  `;
  
  // Add to hero section
  const hero = document.querySelector('.hero-metrics');
  if (hero) {
    hero.parentNode.insertBefore(statusElement, hero.nextSibling);
  }
...
// Enhanced hover effects for professional cards
function initCardHoverEffects() {
  const cards = document.querySelectorAll('.value-card, .comparison-card, .stat-card, .token-card');
...
// Professional timeline/progress visualization
function initProgressVisualization() {
  const progressBars = document.querySelectorAll('.progress-visual');
...
function initProfessionalTooltips() {
...
function initProfessionalLogoAnimations() {
  const logos = document.querySelectorAll('.logo-3d, .chain-logo');
  
  logos.forEach(logo => {
    logo.addEventListener('mouseenter', () => {
      logo.style.filter = 'drop-shadow(0 0 20px rgba(212, 167, 106, 0.6)) brightness(1.2)';
      logo.style.transform = 'scale(1.05)';
    });
    
    logo.addEventListener('mouseleave', () => {
      logo.style.filter = 'drop-shadow(0 0 10px rgba(212, 167, 106, 0.3))';
      logo.style.transform = 'scale(1)';
    });
  });
}

// Add these CSS classes for the new effects
function addProfessionalStyles() {
  ...
  style.textContent = `
    ...
    /* Enhanced typography for professional feel */
    .professional-heading {
      font-size: 2.5rem;
      background: linear-gradient(135deg, var(--rebel-gold), var(--rebel-red));
...
// Add to your initIndexPage function:
function initIndexPage() {
  console.log('✨ Initializing professional investor features...');
  ...
  const professionalFeatures = [
    ...
    initProfessionalTooltips,
    initProfessionalLogoAnimations
  ];
  
  professionalFeatures.forEach((initFn, index) => {
    setTimeout(() => {
      try {
        initFn();
      } catch (error) {
        console.warn(`⚠️ Failed to initialize professional feature ${initFn.name}:`, error);
      }
    }, index * 200);
  });
...
// Add this CSS for professional hover tilt effect
const tiltStyles = `
  .value-card,
  .comparison-card,
  .stat-card,
...
// Add tilt styles to document
if (!document.querySelector('#tilt-styles')) {
  const tiltStyleEl = document.createElement('style');
  tiltStyleEl.id = 'tilt-styles';
  tiltStyleEl.textContent = tiltStyles;
  document.head.appendChild(tiltStyleEl);
}
